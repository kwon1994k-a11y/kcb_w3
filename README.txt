...
경마 현장예측 사용
------------------
1. 앱의 `관리자 업로드` 메뉴로 이동합니다.
2. `경마 현장예측 db 등록`에서 경마 엑셀 파일을 선택합니다.
   - 파일의 `db` 시트에는 `no, a1, a2, a3, a4, a5, n1, n2, n3` 열이 필요합니다.
   - 예: `kra_0524-인&착-db.xlsx`
3. `경마 현장예측 db 저장`을 누릅니다.
4. `경마 현장예측` 메뉴에서 현장 인기도 순서의 말 번호 5개를 입력합니다.
   - 예: `9,6,11,4,5`
5. 저장된 과거 경마 db의 동일 조합 또는 유사 조합 패턴을 바탕으로 `n1~n3` 예측 결과를 확인합니다.

경정 현장예측 사용
------------------
1. 앱의 `관리자 업로드` 메뉴로 이동합니다.
2. `경정 현장예측 db 등록`에서 경정 엑셀 파일을 선택합니다.
   - 파일의 `db` 시트에는 `no, a1, a2, a3, a4, a5, n1, n2, n3` 열이 필요합니다.
   - 예: `kbot_0521-인&착-db.xlsx`
3. `경정 현장예측 db 저장`을 누릅니다.
4. `경정 현장예측` 메뉴에서 현장 인기도 순서의 선수 번호 5개를 입력합니다.
   - 경정 선수 번호는 1~6입니다. 예: `1,3,2,5,6`
5. 저장된 과거 경정 db의 동일 조합 또는 유사 조합 패턴을 바탕으로 `n1~n3` 예측 결과를 확인합니다.
